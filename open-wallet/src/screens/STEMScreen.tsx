...
export function STEMScreen({ onClose }: Props) {
  ...
  const s = useMemo(() => StyleSheet.create({
    ...
    ageChipText: { color: t.text.muted, fontSize: 12, fontWeight: fonts.semibold },
    ...
    badge: { fontSize: 28, marginRight: 12 },
    ...
    mentorField: { color: t.accent.blue, fontSize: 12, fontWeight: fonts.semibold, marginTop: 2 },
    mentorSpec: { color: t.text.secondary, fontSize: 12, marginTop: 2 },
    mentorStats: { flexDirection: 'row', gap: 12, marginTop: 6 },
    mentorStat: { color: t.text.muted, fontSize: 11 },
    ...
    expTitle: { color: t.text.primary, fontSize: 14, fontWeight: fonts.bold },
    expCategory: { color: t.accent.blue, fontSize: 11, fontWeight: fonts.semibold, marginTop: 2 },
    expMeta: { flexDirection: 'row', gap: 12, marginTop: 8 },
    expMetaText: { color: t.text.muted, fontSize: 11 },
    materialsTitle: { color: t.text.secondary, fontSize: 11, fontWeight: fonts.semibold, marginTop: 10, marginBottom: 4 },
    ...
    materialText: { color: t.text.secondary, fontSize: 11 },
    fairCard: { backgroundColor: t.accent.purple + '10', borderRadius: 20, padding: 20, marginBottom: 16 },
    fairTitle: { color: t.text.primary, fontSize: 17, fontWeight: fonts.heavy, textAlign: 'center', marginTop: 8 },
    fairDate: { color: t.accent.purple, fontSize: 13, fontWeight: fonts.semibold, textAlign: 'center', marginTop: 4 },
    fairStatus: { color: t.accent.orange, fontSize: 12, fontWeight: fonts.bold, textAlign: 'center', marginTop: 4, textTransform: 'uppercase' },
    fairStats: { flexDirection: 'row', justifyContent: 'space-around', marginTop: 16 },
    fairStatValue: { color: t.text.primary, fontSize: 22, fontWeight: fonts.heavy, textAlign: 'center' },
    fairStatLabel: { color: t.text.muted, fontSize: 11, textAlign: 'center', marginTop: 2 },
    ...
    <View style={s.section}>
      ...
      {DEMO_MENTORS.map((mentor) => (
        <View key={mentor.id} style={s.mentorCard}>
          <Text style={{ fontSize: 36 }}>{mentor.icon}</Text>
      ...
      {filteredExperiments.map((exp) => (
        <View key={exp.id} style={s.expCard}>
          <View style={s.expHeader}>
            <Text style={{ fontSize: 32 }}>{exp.icon}</Text>
