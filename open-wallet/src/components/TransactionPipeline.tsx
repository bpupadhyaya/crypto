...
export function TransactionPipeline({ steps, title, fromSymbol, toSymbol, amount, provider }: Props) {
  ...
  const s = useMemo(() => StyleSheet.create({
    ...
    subtitle: { color: t.text.muted, fontSize: fonts.sm, marginTop: 4 },
    swapSummary: {
      flexDirection: 'row', alignItems: 'center', justifyContent: 'center',
      backgroundColor: t.bg.card, borderRadius: 12, padding: 14, marginBottom: 20, gap: 8,
    },
    swapToken: { color: t.text.primary, fontSize: fonts.lg, fontWeight: fonts.bold },
    swapArrow: { color: t.text.muted, fontSize: fonts.lg },
    providerBadge: {
      alignSelf: 'center', backgroundColor: t.accent.blue + '15',
      borderRadius: 8, paddingHorizontal: 12, paddingVertical: 4, marginBottom: 20,
    },
    providerText: { color: t.accent.blue, fontSize: fonts.sm, fontWeight: fonts.semibold },
    // Pipeline
    ...
    dotText: { fontSize: fonts.sm, fontWeight: fonts.heavy },
    line: { width: 2, flex: 1, marginVertical: 2 },
    contentCol: { flex: 1, paddingLeft: 12, paddingBottom: 16 },
    stepLabel: { fontSize: fonts.md, fontWeight: fonts.semibold },
    stepDetail: { fontSize: fonts.sm, marginTop: 2, lineHeight: 17 },
    stepInfo: { fontSize: fonts.xs, marginTop: 3, fontFamily: 'Courier' },
  }), [t]);
