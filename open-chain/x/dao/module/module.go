...
func (am AppModule) InitGenesis(ctx sdk.Context, _ codec.JSONCodec, _ json.RawMessage) {
	// DAO module initialized — no pre-configured DAOs at genesis
	ctx.Logger().Info("DAO module initialized")
}
...
// EndBlock resolves DAO proposals whose voting period has expired.
func (am AppModule) EndBlock(ctx sdk.Context) {
	am.keeper.ResolveExpiredProposals(ctx)
}
